...
pub unsafe extern "C" fn CreateHeap() -> MinHeap {
    ...
    (*H)
        .Data = malloc(
        ((1001 as libc::c_int + 1 as libc::c_int) as libc::c_ulong)
            .wrapping_mul(::core::mem::size_of::<HuffmanTree>() as libc::c_ulong),
    ) as *mut HuffmanTree;
    (*H).Size = 0 as libc::c_int;
    (*H).Capacity = 1001 as libc::c_int;
...
unsafe fn main_0(
    ...
    let mut N: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    let mut f: [libc::c_int; 1001] = [0; 1001];
    let mut c: [libc::c_char; 1001] = [0; 1001];
    let mut HFT: HuffmanTree = 0 as *mut HTNode;
    mini = CreateHuffman();
    ...
    HFT = Huffman(H);
    printf(b"%d\n\0" as *const u8 as *const libc::c_char, (*HFT).Weight);
