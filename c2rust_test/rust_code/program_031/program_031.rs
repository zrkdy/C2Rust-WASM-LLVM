...
#[no_mangle]
pub static mut stack: [*mut Node; 10000] = [0 as *const Node as *mut Node; 10000];
#[no_mangle]
...
pub unsafe extern "C" fn hillclimbing(
    ...
    let mut nodes: [*mut Node; 4] = [0 as *mut Node; 4];
    let mut nodesszie: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    while empty() == 0 {
        counter += 1 as libc::c_int;
        currentnode = top();
        cube_display(((*currentnode).cube).as_mut_ptr());
        printf(b"************\n\0" as *const u8 as *const libc::c_char);
        if cube_equal(((*currentnode).cube).as_mut_ptr(), target) != 0 {
            ...
            if (*currentnode).x > 0 as libc::c_int {
                ...
                if inme(currentnode, tmpcube.as_mut_ptr()) == 0 {
                    ...
                    let fresh3 = nodesszie;
                    nodesszie = nodesszie + 1;
                    nodes[fresh3 as usize] = node;
                }
            }
            cube_copy(((*currentnode).cube).as_mut_ptr(), tmpcube.as_mut_ptr());
            if (*currentnode).x < 3 as libc::c_int - 1 as libc::c_int {
                ...
                if inme(currentnode, tmpcube.as_mut_ptr()) == 0 {
                    ...
                    (*node).prev = currentnode;
                    (*node).weight = calc_weight(target, ((*node).cube).as_mut_ptr());
                    let fresh4 = nodesszie;
                    nodesszie = nodesszie + 1;
                    nodes[fresh4 as usize] = node;
                }
            }
            cube_copy(((*currentnode).cube).as_mut_ptr(), tmpcube.as_mut_ptr());
            if (*currentnode).y > 0 as libc::c_int {
                ...
                if inme(currentnode, tmpcube.as_mut_ptr()) == 0 {
                    ...
                    (*node).prev = currentnode;
                    (*node).weight = calc_weight(target, ((*node).cube).as_mut_ptr());
                    let fresh5 = nodesszie;
                    nodesszie = nodesszie + 1;
                    nodes[fresh5 as usize] = node;
                }
            }
            cube_copy(((*currentnode).cube).as_mut_ptr(), tmpcube.as_mut_ptr());
            if (*currentnode).y < 3 as libc::c_int - 1 as libc::c_int {
                ...
                if inme(currentnode, tmpcube.as_mut_ptr()) == 0 {
                    ...
                    (*node).prev = currentnode;
                    (*node).weight = calc_weight(target, ((*node).cube).as_mut_ptr());
                    let fresh6 = nodesszie;
                    nodesszie = nodesszie + 1;
                    nodes[fresh6 as usize] = node;
                }
            }
            i = 0 as libc::c_int;
            while i < nodesszie {
                push(nodes[i as usize]);
                i += 1;
                i;
            }
        }
