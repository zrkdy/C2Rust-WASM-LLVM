...
pub unsafe extern "C" fn print_list() {
    printf(b"FORWARD order print:\n\0" as *const u8 as *const libc::c_char);
    print_forward_order();
    printf(b"REVERSE order print:\n\0" as *const u8 as *const libc::c_char);
    print_reverse_order();
}
#[no_mangle]
pub unsafe extern "C" fn print_forward_order() {
    if head.is_null() {
        return;
    }
    let mut current: *mut node = head;
...
#[no_mangle]
pub unsafe extern "C" fn print_reverse_order() {
    if head.is_null() {
        return;
    }
    let mut current: *mut node = tail;
    loop {
        printf(b"%d \0" as *const u8 as *const libc::c_char, (*current).number);
