        ...
        if cube_equal(((*currentnode).cube).as_mut_ptr(), target) != 0 {
            ...
            if (*currentnode).x > 0 as libc::c_int {
                ...
                if inme(currentnode, tmpcube.as_mut_ptr()) == 0 {
                    ...
                    (*node).g = (*currentnode).g + 1 as libc::c_int;
                    h = calc_weight(target, ((*node).cube).as_mut_ptr());
                    (*node).f = (*node).g + h;
...
unsafe fn main_0(
    ...
    astar(origin.as_mut_ptr(), x, y, target.as_mut_ptr());
